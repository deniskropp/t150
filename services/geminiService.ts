...
/**
 * Generates an SVG string based on the user's prompt and selected options.
 * Uses 'gemini-3-pro' as requested for generation.
 */
export const generateSvgFromPrompt = async (prompt: string, options: SvgOptions): Promise<string> => {
  try {
    const systemPrompt = `
    ...
    const fullPrompt = `
      Create an SVG representation of the following object/item: "${prompt}".
      
      Please strictly adhere to these specific design attributes:
      - **Art Style**: ${options.style}
      - **Color Palette**: ${options.palette}
      - **Complexity/Detail**: ${options.complexity}
    `;
