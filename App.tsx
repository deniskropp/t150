...
import { GeneratedSvg, GenerationStatus, ApiError, SvgOptions } from './types';
...
const App: React.FC = () => {
  ...
  const handleGenerate = async (prompt: string, options: SvgOptions) => {
    setStatus(GenerationStatus.LOADING);
    setError(null);
    setCurrentSvg(null);

    try {
      const svgContent = await generateSvgFromPrompt(prompt, options);
